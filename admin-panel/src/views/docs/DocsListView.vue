<template>
  <div class="module-page">
    <CommandPanel title="文档管理" subtitle="DOCS MANAGEMENT">
      <div class="module-toolbar">
        <input v-model="keyword" class="module-input" type="text" placeholder="按标题或分组搜索" />
        <RouterLink class="command-btn command-btn--primary" to="/docs/create">新建文档</RouterLink>
      </div>
            ...
            <tr v-for="item in filteredDocs" :key="item.id">
              ...
              <td class="action-col">
                <div class="table-actions">
                  <RouterLink class="text-link" :to="`/docs/${item.id}/edit`">编辑</RouterLink>
...
import { deleteDoc, fetchDocsList, moveDocOrder } from '../../services/docsService'
...
const filteredDocs = computed(() => {
  ...
  return list.value.filter((item) => {
    return String(item.title).toLowerCase().includes(query) || String(item.group).toLowerCase().includes(query)
  })
})

async function load() {
  list.value = await fetchDocsList()
}
...
function enqueueMove(id, action) {
  ...
  moveQueue = moveQueue
    .then(() => moveDocOrder(id, action))
    .catch(async (error) => {
      console.error('文档排序同步失败', error)
      await load()
    })
    .finally(async () => {

...
async function persistResources(list, changedIds = new Set()) {
  const normalized = withSequentialOrder(list, changedIds)
  await replaceEntityList('resources', normalized)
  return normalized
}
...
export async function fetchResourcesList() {
  const items = await listEntity('resources')
  return items.sort((a, b) => Number(a.order || 0) - Number(b.order || 0))
}

export async function fetchResourceById(id) {
  return getEntity('resources', id)
}

export async function saveResource(payload) {
  const list = await fetchResourcesList()
  ...
  if (isEdit) {
    ...
    return (await persistResources(orderedOthers, changedIds)).find(
  ...
  return (await persistResources(ordered, changedIds)).find(
    (item) => Number(item.id) === Number(created.id),
  )
}

export async function deleteResource(id) {
  await removeEntity('resources', id)
  const current = await fetchResourcesList()
  await persistResources(current)
  return { ok: true }
}

export async function moveResourceOrder(id, action) {
  const list = sortByOrder(await fetchResourcesList())
  ...
  const normalized = await persistResources(resequenced, changedIds)
